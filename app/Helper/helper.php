...
function get_option($user_id, $key,$default=null)
{
    $cacheKey = 'setting_' . $user_id . '_' . $key;
    return gCache::remember($cacheKey, 60, function () use ($user_id, $key, $default) {
...
function decrypt_cipher($cipherText)
{
...
function generateTransactionRefId($length = 14)
{
...
function saveSpotify($data, $token = null, $user_location_id = '')
{
    ...
    if ($token) {
        ...
        if ($profileData) {
            ...
            $spotifyData = array_merge($spotifyData, [
                'spotify_auth_id' => $spotifyid,
